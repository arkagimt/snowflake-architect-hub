          ...
          <div className="flex gap-3">
            <button
              ...
              className={`px-5 py-2 rounded-lg font-semibold transition flex items-center gap-2 ${
                isAnimating ? 'bg-yellow-600 text-white animate-pulse' :
                      ...
                      return (
                        <div
                          ...
                          className={`p-2 rounded text-xs flex justify-between items-center transition ${
                            isHighlighted ? 'bg-emerald-500/30 border border-emerald-400' : 'bg-slate-800/50'
                          }`}
                        >
                          <div className="flex gap-3">
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-lg transition ${
                          isConnected ? 'bg-orange-500/30 border border-orange-400' : 'bg-slate-800/50'
                        }`}
                      >
                        <div className="flex justify-between items-center">
                          <div>
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-700 p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-xs px-2 py-1 rounded ${
                isClustered ? 'bg-green-900/50 text-green-400' : 'bg-red-900/50 text-red-400'
              }`}>
            ...
            <div className="relative h-64 bg-slate-950 rounded-lg p-3">
              {currentData.map((partition, idx) => {
                ...
                return (
                  <div
                    key={partition.id}
                    className={`absolute h-8 rounded-md transition-all duration-1000 flex items-center justify-center text-[10px] font-bold text-white ${
                      isScanned ? 'ring-2 ring-yellow-400 ring-offset-2 ring-offset-slate-950 z-10' : ''
              ...
              <div className="grid grid-cols-6 gap-2">
                {[5, 10, 12, 15, 20, 25].map(date => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm font-mono transition ${
                      searchDate === date 
                ...
                <div className="mt-4 p-3 rounded-lg bg-slate-800 border border-slate-700">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-lg font-bold ${
                      scannedPartitions.length <= 1 ? 'text-green-400' :
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-lg font-bold ${
                      pruningEfficiency >= 80 ? 'text-green-400' :
                  ...
                  <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-500 ${
                        pruningEfficiency >= 80 ? 'bg-green-500' :
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center p-3 bg-slate-800 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-xl font-bold mono ${
                      isClustered ? 'text-green-400' : 'text-red-400'
                    }`}>
                ...
                <div className="flex justify-between items-center p-3 bg-slate-800 rounded-lg">
                  ...
                  <span className={`text-sm font-bold px-2 py-1 rounded ${
                    isClustered ? 'bg-green-900/50 text-green-400' : 'bg-yellow-900/50 text-yellow-400'
                  }`}>
                ...
                <div className="bg-slate-900 rounded-xl p-4 mb-4 border border-slate-700">
                  ...
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div className={`px-4 py-2 rounded-lg border-2 ${
                      searchSteps.some(s => s.includes('ROOT')) ? 'border-green-500 bg-green-900/30' : 'border-slate-600 bg-slate-800'
                    }`}>
                    ...
                    <div className="flex gap-4">
                      {[
                        ...
                        <div key={idx} className={`px-3 py-2 rounded-lg border-2 ${
                          leaf.highlight && searchSteps.length > 2 ? 'border-green-500 bg-green-900/30' : 'border-slate-600 bg-slate-800'
                        }`}>
                ...
                <div className="bg-slate-900 rounded-xl p-4 mb-4 border border-slate-700">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {sampleData.map((row, idx) => (
                      <div key={row.id} className={`p-2 rounded border text-xs ${
                        searchSteps.some(s => s.includes(`ID = ${row.id}`) && s.includes('FOUND'))
                  ...
                  {[1, 3, 5, 8, 13, 21, 99].map(id => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-sm font-mono transition ${
                        searchId === id ? 'bg-green-600 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'
                      }`}
                    >
                      {id}
                    </button>
                ...
                <div className="bg-slate-900 rounded-xl p-4 border border-slate-700">
                  ...
                  <div className="space-y-1 mono text-xs">
                    {searchSteps.map((step, idx) => (
                      <div key={idx} className={`py-1 animate-slide ${
...
const FinOpsControlCenter = () => {
  ...
  return (
    <div className={`h-full p-6 overflow-y-auto transition-all ${
      budgetExceeded ? 'ring-4 ring-red-500 animate-pulse' : ''
    }`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-700 p-6 mb-6">
              ...
              {/* Big Cost Display */}
              <div className={`text-center p-6 rounded-xl mb-4 transition-all ${
                isIdleBilling ? 'bg-red-900/30 border-2 border-red-500' :
                isProcessing ? 'bg-green-900/30 border-2 border-green-500' :
                'bg-slate-800 border-2 border-slate-700'
              }`}>
                <div className="text-xs text-slate-500 uppercase mb-1">
                  {isIdleBilling ? '🔴 IDLE TIME BILLING (WASTE)' :
                ...
                <div className="text-sm text-slate-400 mt-2">
                  {elapsedTime}s elapsed • {currentWH.name} @ ${warehouseSize}/hr
                  {serverlessMode && <span className="text-cyan-400 ml-2">⚡ Serverless 1.25x</span>}
                </div>
              </div>
                  ...
                  <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all ${
                        selectedAlgorithm === 'hash' ? 'bg-green-500' : 'bg-red-500'
                      }`}
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-3 rounded-lg font-bold transition ${
                    isProcessing || isIdleBilling ? 'bg-yellow-600 animate-pulse' : 'bg-emerald-600 hover:bg-emerald-500'
                  } text-white`}
                >
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-700 p-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className={`ml-auto px-3 py-1 rounded-lg text-sm font-bold ${
                  cost > maxBudget ? 'bg-red-600 text-white' :
              ...
              <div className="relative bg-slate-800 rounded-xl p-6 mb-4">
                <div className="flex items-center justify-around">
                  ...
                  <div className={`w-20 h-20 rounded-xl flex items-center justify-center transition-all ${
                    biMode === 'import' ? 'bg-green-900/50 border-2 border-green-500' : 'bg-orange-900/50 border-2 border-orange-500'
                  }`}>
                ...
                return (
                  ...
                  >
                    ...
                    {hasBeenScanned && !isCurrentlyScanng && (
                      <div className={`absolute -top-3 left-1/2 -translate-x-1/2 text-xs font-bold px-2 py-0.5 rounded ${
                        isInRange
                          ? 'bg-green-500 text-white'
                          : 'bg-red-500 text-white'
                      }`}>
                        {isInRange ? '✓ SCAN' : '✗ PRUNED'}
                      </div>
                    )}
                    ...
                    <div className="text-center">
                      <div className={`text-lg font-bold ${
                        hasBeenScanned && !isInRange ? 'text-slate-600 line-through' : 'text-cyan-400'
                      }`}>
          ...
          {scanComplete && (
            <div className="grid grid-cols-4 gap-4 mb-6 animate-slide">
              ...
              <div className="bg-purple-900/30 border border-purple-500/30 rounded-xl p-5 text-center">
                ...
                <div className="text-xs text-green-400 mt-1">🚀 {formatRows(stats.prunedRows)} rows saved!</div>
              </div>
            </div>
          )}
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          <div className="max-w-4xl mx-auto">
                  ...
                  <div className="space-y-2">
                    {tableA.map((row, idx) => (
                      <div
                        key={idx}
                        data-row-a={idx}
                        className={`relative h-10 flex items-center justify-center rounded-lg border-2 font-mono text-sm transition-all cursor-pointer ${getValueColor(row.display, 'bg')} ${getValueColor(row.display, 'border')} ${
                          highlightValue === row.display ? 'ring-2 ring-white scale-105 z-10' : ''
                        }`}
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              {/* The 0 Explosion */}
              <div className={`bg-yellow-900/20 border rounded-xl p-4 transition-all ${
                highlightValue === '0' ? 'border-yellow-400 ring-2 ring-yellow-400/30' : 'border-yellow-500/30'
              }`}
              ...
              {/* The NULL = NULL Trap */}
              <div className={`bg-red-900/20 border rounded-xl p-4 transition-all ${
                highlightValue === 'NULL' ? 'border-red-400 ring-2 ring-red-400/30' : 'border-red-500/30'
              }`}
              ...
              <div className={`bg-orange-900/20 border rounded-xl p-4 transition-all ${
                nullValueAnim?.active ? 'border-orange-400 ring-2 ring-orange-400/30 animate-pulse' : 'border-orange-500/30'
              }`}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">🚫</span>
              ...
              {/* The 2 Multiplier */}
              <div className={`bg-green-900/20 border rounded-xl p-4 transition-all ${
                highlightValue === '2' ? 'border-green-400 ring-2 ring-green-400/30' : 'border-green-500/30'
              }`}
...
-- Every row × every row = ${tableA.length} × ${tableB.length} = ${tableA.length * tableB.length} rows!`}
        ...
        <div className="w-80 bg-slate-900 border-l border-slate-800 p-4 overflow-y-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className={`text-sm font-bold px-2 py-1 rounded ${
              joinType === 'cross' ? 'bg-red-900/50 text-red-400' :
          ...
          <div className="space-y-1 max-h-[400px] overflow-y-auto">
            {results.slice(0, 40).map((row, idx) => (
              <div key={idx} className="grid grid-cols-2 gap-1">
                <div className={`p-2 rounded text-center font-mono text-xs ${
                  row.isGhost && row.aId === null
          ...
          <div className="mt-4 p-3 bg-slate-800 rounded-xl text-sm">
            ...
            {joinType !== 'cross' ? (
              ...
              <div className="text-center font-mono">
                ...
                <div className="text-xs text-red-400 mt-1">⚠️ Cartesian!</div>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 p-3 bg-cyan-900/20 border border-cyan-500/30 rounded-xl">
            <div className="text-xs font-bold text-cyan-400 mb-1">🎯 Interview Tip</div>
            <div className="text-xs text-slate-300">
              {joinType === 'inner' && "NULL never equals ANYTHING - not even itself! NULL = NULL → FALSE, NULL = '4' → FALSE. This is why NULLs are excluded from INNER JOINs. Watch the orange scanning animation!"}
...
const WindowSimulator = ({ onBack }: { onBack: () => void }) => {
  ...
  const teamsData = [
    { id: 1, team: 'India', flag: '🇮🇳', points: 10, nrr: 1.2, group: 'A', color: 'blue' },
    { id: 2, team: 'Australia', flag: '🇦🇺', points: 10, nrr: 0.8, group: 'A', color: 'yellow' },
    { id: 3, team: 'New Zealand', flag: '🇳🇿', points: 10, nrr: 0.5, group: 'A', color: 'gray' },
    { id: 4, team: 'England', flag: '🏴󠁧󠁢󠁥󠁮󠁧󠁿', points: 8, nrr: 0.3, group: 'A', color: 'red' },
    { id: 5, team: 'Pakistan', flag: '🇵🇰', points: 8, nrr: 0.1, group: 'B', color: 'green' },
    { id: 6, team: 'South Africa', flag: '🇿🇦', points: 6, nrr: -0.2, group: 'B', color: 'emerald' },
  ];
      ...
      <header className="bg-slate-900/80 backdrop-blur border-b border-slate-800 px-6 py-4 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">ROW_NUMBER • RANK • DENSE_RANK • LEAD • LAG</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-yellow-900/30 to-orange-900/30 border border-yellow-500/30 rounded-xl p-5 mb-6">
            <h3 className="text-lg font-bold text-white mb-2">
              ...
              {windowFunc === 'rank' && '🏆 RANK() - Ties Get Same Rank, Then Skip'}
              {windowFunc === 'dense_rank' && '🎖️ DENSE_RANK() - Ties Get Same Rank, No Skip'}
              {windowFunc === 'lead' && '⏭️ LEAD() - Peek at Next Row'}
              {windowFunc === 'lag' && '⏮️ LAG() - Peek at Previous Row'}
            </h3>
            <p className="text-sm text-slate-300">
              {windowFunc === 'row_number' && 'Assigns 1, 2, 3... to each row. Ties are broken arbitrarily. Each row gets a UNIQUE number.'}
              {windowFunc === 'rank' && 'Same value = same rank. But the NEXT rank SKIPS! (1, 1, 1 → 4). Like Olympic medals with missing 2nd & 3rd.'}
              {windowFunc === 'dense_rank' && 'Same value = same rank. The NEXT rank is sequential! (1, 1, 1 → 2). No gaps in ranking.'}
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-semibold transition ${
                  isSorting ? 'bg-yellow-600 animate-pulse' : 'bg-green-600 hover:bg-green-500'
                } text-white`}
              >
              ...
              {sortedData.map((team, idx) => {
                ...
                return (
                  <div key={team.id}>
                    ...
                    <div 
                      className={`grid grid-cols-12 gap-2 items-center p-3 rounded-xl border-2 transition-all ${
                        isSorting ? 'animate-pulse' : ''
                      } ${getTeamBgColor(team.color)} ${
                        isTied && showResults ? 'ring-2 ring-yellow-400/50' : ''
                      }`}
                      style={{
                    ...
                    >
                      ...
                      <div className="text-center">
                        <span className={`text-xs px-2 py-0.5 rounded ${
                          team.group === 'A' ? 'bg-blue-900/50 text-blue-400' : 'bg-green-900/50 text-green-400'
                        }`}>
                          {team.group}
                        </span>
                      </div>
                      ...
                      <div className="col-span-2 text-center relative">
                        {showResults && (
                          <div className={`inline-flex items-center justify-center w-12 h-10 rounded-lg font-bold text-lg animate-slide ${
          ...
          {/* Comparison Grid */}
          <div className="bg-slate-900 rounded-xl border border-slate-700 p-5">
            <h4 className="font-bold text-white mb-4">📊 Quick Comparison: Tie Behavior</h4>
        ...
        <div className="flex items-center justify-center gap-8 mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-bold transition ${
                  isNull ? 'bg-red-600 text-white' : 'bg-slate-700 text-slate-400 hover:bg-red-900/50'
                }`}
              >
                👻 NULL
              </button>
          ...
          <button
            ...
            className={`px-6 py-3 rounded-lg font-bold transition ${
              isAnimating ? 'bg-yellow-600 animate-pulse' : 'bg-green-600 hover:bg-green-500'
            } text-white`}
          >
        ...
        <div className="relative bg-slate-800 rounded-xl p-8">
          ...
          <div className="relative h-24">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
              <div className={`w-20 h-20 rounded-xl border-4 flex flex-col items-center justify-center transition-all ${
                isAnimating && boxPosition >= 45 && boxPosition <= 55 && isNull
            ...
            {(isAnimating || showResult) && (
              <div
                className={`absolute top-1/2 -translate-y-1/2 w-16 h-16 rounded-xl flex items-center justify-center font-bold text-lg transition-all z-10 ${
                  (isNull && !isTransformed)
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700 p-6 mb-6">
        <div className="flex items-center justify-center gap-6 mb-6">
          ...
          <button
            ...
            disabled={isAnimating}
            className={`px-6 py-3 rounded-lg font-bold transition ${
              isAnimating ? 'bg-yellow-600 animate-pulse' : 'bg-cyan-600 hover:bg-cyan-500'
            } text-white`}
          >
        ...
        <div className="flex justify-center">
          <div className="relative w-80">
            ...
            <div className="flex flex-col items-center">
              ...
              {columns.map((col, idx) => {
                ...
                return (
                  <div key={col.id} className="relative my-4">
                    {/* Layer Platform */}
                    <div className={`relative w-48 h-16 rounded-xl border-2 flex items-center justify-center transition-all ${
                      isStopPoint
                      ...
                      {/* Trapdoor Animation */}
                      <div className={`absolute inset-2 flex transition-all duration-300 ${
                        isTrapdoorOpen ? 'gap-16' : 'gap-0'
                      }`}>
                        <div className={`flex-1 h-full rounded bg-slate-600 transition-all duration-300 ${
                          isTrapdoorOpen ? 'rotate-[-30deg] origin-left' : ''
                        }`} />
                        <div className={`flex-1 h-full rounded bg-slate-600 transition-all duration-300 ${
                          isTrapdoorOpen ? 'rotate-[30deg] origin-right' : ''
                        }`} />
                      </div>

                      {/* Value Label */}
                      <span className={`relative z-10 font-bold text-lg ${
                        col.value !== null ? 'text-green-400' : 'text-red-400'
                      }`}>
                      ...
                      {isCurrent && !isAnimating && showResult && (
                        <span className={`absolute -right-8 text-xl ${
                          col.value !== null ? 'text-green-400' : 'text-red-400'
                        }`}>
                          {col.value !== null ? '✓' : '↓'}
                        </span>
              ...
              {/* NULL Bucket */}
              <div className={`w-48 h-16 mt-4 rounded-xl border-2 flex items-center justify-center transition-all ${
                stoppedAt === null && showResult
                  ? 'bg-red-900/50 border-red-500 shadow-lg shadow-red-500/30'
                  : 'bg-slate-800 border-slate-600 border-dashed'
              }`}>
                <span className="text-red-400 font-bold">NULL Bucket 🪣</span>
              </div>
    ...
    <div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700 p-8 mb-6">
        ...
        <div className="flex items-center justify-center gap-6 py-8">
          {/* Input */}
          <div className={`w-24 h-24 rounded-2xl flex items-center justify-center font-bold text-3xl transition-all ${
            isNull
        ...
        <div className={`text-center p-4 rounded-xl transition-all ${
          isNull ? 'bg-red-900/20 border border-red-500/30' : 'bg-green-900/20 border border-green-500/30'
        }`}>
          {isNull ? (
            <div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-700 p-6 mb-6">
        ...
        <div className="flex justify-center gap-4 mb-8">
          <button
            ...
            disabled={isScanning}
            className={`px-6 py-3 rounded-lg font-bold transition ${
              isScanning && countType === 'star' ? 'bg-yellow-600 animate-pulse' : 'bg-yellow-600 hover:bg-yellow-500'
            } text-white`}
          >
            Run COUNT(*)
          </button>
          <button
            ...
            disabled={isScanning}
            className={`px-6 py-3 rounded-lg font-bold transition ${
              isScanning && countType === 'col' ? 'bg-cyan-600 animate-pulse' : 'bg-cyan-600 hover:bg-cyan-500'
            } text-white`}
          >
            Run COUNT(column)
          </button>
        ...
        <div className="flex justify-center gap-4 mb-8">
          {dataBlocks.map((block, idx) => {
            ...
            return (
              <div
                key={block.id}
                className={`relative w-20 h-20 rounded-xl flex items-center justify-center font-bold text-xl transition-all ${
                  isCurrentScan
                    ? flashState === 'green'
        ...
        {/* Result Counter */}
        <div className="flex justify-center">
          <div className={`inline-flex items-center gap-4 px-8 py-4 rounded-xl border-2 ${
            countResult !== null
              ? countType === 'star'
            ...
            <span className={`text-4xl font-bold ${
              countType === 'star' ? 'text-yellow-400' : 'text-cyan-400'
            }`}>
              {countResult ?? '?'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-slate-900 rounded-xl border-2 border-teal-500/50 p-5 relative">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`text-xs px-2 py-0.5 rounded ${
                  streamData.length > 0 ? 'bg-teal-600 text-white' : 'bg-slate-700 text-slate-400'
                }`}>
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-bold ${
                        record.action === 'INSERT' ? 'text-green-400' : 'text-red-400'
                      }`}>
                        {record.action}
                      </span>
            ...
            <div className="bg-slate-900 rounded-xl border border-slate-700 p-5">
              ...
              <button
                ...
                className={`w-full px-4 py-3 rounded-lg font-bold text-sm transition mb-4 ${
                  isMerging
              ...
              <div className="space-y-1 max-h-[250px] overflow-y-auto">
                ...
                {targetRows.map(row => {
                  ...
                  return (
                    <div
                      key={row.id}
                      className={`grid grid-cols-4 gap-1 text-xs rounded-lg px-2 py-2 border transition-all ${
                        isBeingUpdated
...
const CostSimulator = ({ onBack }: { onBack: () => void }) => {
  ...
  const tabs = [
    ...
    { id: 'matillion', label: 'Matillion ETL', icon: '🟠', desc: 'Self-Hosted vs SaaS' },
        ...
        <div className="flex gap-2 justify-center">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={`px-5 py-3 rounded-xl text-sm font-semibold transition flex flex-col items-center gap-1 ${
                activeTab === tab.id
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-700 p-6">
          ...
          {/* Status Display */}
          <div className={`p-6 rounded-xl text-center border-2 mb-4 transition-all ${
            isRunning ? 'border-green-500 bg-green-900/20 shadow-lg shadow-green-500/20' :
            ...
            <div className={`text-xl font-bold ${
              isRunning ? 'text-green-400' : isIdle ? 'text-red-400' : 'text-slate-400'
            }`}>
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-3 rounded-lg font-bold text-white transition ${
                isRunning || isIdle ? 'bg-slate-700 opacity-50' : 'bg-emerald-600 hover:bg-emerald-500'
              }`}
            >
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-700 p-6">
          ...
          {/* Server Visual */}
          <div className={`relative h-48 rounded-xl border-2 flex items-center justify-center mb-4 transition-all ${
            mode === 'self-hosted'
            ...
            <div className="text-center">
              ...
              <div className={`font-bold ${
                mode === 'self-hosted' ? 'text-orange-400' :
            ...
            <div className={`absolute top-4 right-4 w-4 h-4 rounded-full ${
              mode === 'self-hosted' ? 'bg-green-500 animate-pulse' :
          ...
          {mode === 'saas' && (
            <button
              ...
              className={`w-full py-3 rounded-lg font-bold text-white transition ${
                isRunning ? 'bg-cyan-600 animate-pulse' : 'bg-cyan-600 hover:bg-cyan-500'
              }`}
            >
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 rounded-xl border border-slate-700 p-6">
          <h3 className="text-lg font-bold text-white mb-4">🏗️ Architecture Flow</h3>
          ...
          <div className="relative h-64 bg-slate-950 rounded-xl border border-slate-800 p-4">
            ...
            {/* Power BI VertiPaq */}
            <div className={`absolute left-1/3 top-1/2 -translate-y-1/2 -translate-x-1/2 p-4 rounded-xl border-2 transition-all ${
            ...
            {/* Snowflake */}
            <div className={`absolute right-8 top-1/2 -translate-y-1/2 p-4 rounded-xl border-2 transition-all ${
              warehouseActive
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <button
              ...
              className={`py-3 rounded-lg font-bold text-white transition ${
                mode === 'import' ? 'bg-green-600 hover:bg-green-500' : 'bg-orange-600 hover:bg-orange-500'
              }`}
            >
              🖱️ 50 User Clicks
            </button>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-900 rounded-xl border border-slate-700 p-4">
            ...
            <div className={`text-4xl font-bold font-mono text-center p-4 rounded-lg ${
              snowflakeCost > 0.5 ? 'bg-red-900/30 text-red-400' : 'bg-green-900/30 text-green-400'
            }`}>
          ...
          <div className={`rounded-xl border p-4 ${
            mode === 'import' ? 'bg-green-900/20 border-green-500/30' :
            mode === 'directquery' ? 'bg-red-900/20 border-red-500/30' :
            'bg-purple-900/20 border-purple-500/30'
          }`}>
            <h4 className={`font-bold mb-2 ${
              mode === 'import' ? 'text-green-400' :
